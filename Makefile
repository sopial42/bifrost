...
ci-integration: build $(VENOM)
	...
	@mkdir -p ./build
	@echo "🔸 Start server...";
	@./build/$(APPNAME).test -test.coverprofile=./build/$(APPNAME).venom.cover.out > ./build/$(APPNAME).log 2>&1 &
	@sleep 5;
	@echo "🔸 Run integration tests...";
	@$(MAKE) env=$(env) test_suite="$(test_suite_dir)/**/$(integration_test_suite)" venom;
	@echo "🔸 Kill server";
	...
	ls -l ./build/
	go tool cover -html=./build/$(APPNAME).venom.cover.out -o ./build/$(APPNAME).venom.cover.html;
	@echo "🔸 Done";
...
ifndef name
	...
	--push -t $(registry)/$(name):$(tag) -t $(registry)/$(name):latest .
